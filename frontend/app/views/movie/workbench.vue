  ...
  <div class="studio" v-if="drama">
    <header class="studio-topbar">
      <div class="studio-topbar-main">
        <button class="back-btn topbar-back" @click="navigateTo('/movies')">
            ...
            <div class="prod-section-bar">
              ...
              <div class="ml-auto flex gap-1">
                ...
                <button class="btn btn-sm" :disabled="!sbs.length" @click="batchVideos">
                  ...
                  {{ t('dramaEpisode.videos.batchVideo') }}
                </button>
              </div>
            </div>
            ...
            <div v-else class="video-task-workbench has-player">
              <section class="video-task-list">
                ...
                <div>
                  ...
                  <div class="video-task-meta">{{ t('dramaEpisode.videos.taskListMeta', { n: videoTaskRows.length }) }}</div>
                </div>
                <div class="video-task-metrics">
                  <span class="video-task-metric is-pending">{{ t('dramaEpisode.videos.generatingCount', { n: pendingVideoIds.length }) }}</span>
                  <span class="video-task-metric is-done">{{ t('dramaEpisode.videos.doneCount', { n: videoTaskDoneCount }) }}</span>
                  <span class="video-task-metric is-failed">{{ t('dramaEpisode.videos.failedCount', { n: videoTaskFailedCount }) }}</span>
                </div>
                </div>
                  ...
                  <section class="video-inspector-section">
                    ...
                    <div class="video-param-row">
                      ...
                      <span class="video-param-control">
                        ...
                        <span class="video-param-unit">{{ t('dramaEpisode.videos.genDurationUnit') }}</span>
                      </span>
                    </div>
                  </section>
...
// 短片是单集概念，创建项目时后端已自动建好唯一一集（episode_number 固定为 1）
...
function doRewrite() { saveRaw(); runAgent('script_rewriter_short', '请读取剧本并改写为格式化剧本，然后保存', dramaId, epId.value, refresh, chatModelOverride(), chatConfigId()) }
...
function doBreakdown() {
  ...
  const propList = propItems.value.length
    ...
    : '（当前集还没有道具）'
  runAgent('storyboard_breaker_short', `请基于当前集剧本拆分分镜（不需要生成视频提示词，video_prompt 在视频生成阶段按需生成）。
...
function uploadRefMedia(kind) {
  ...
  if (list.value.length >= 3) { toast.info(t('dramaEpisode.toast.refMediaLimit', { label })); return }
...
async function genVid(sb) {
  ...
  try {
    ...
    toast.success(t('dramaEpisode.toast.videoGenerating'))
    await refresh()
  ...
  for (let i = 0; i < 120; i++) {
    ...
    try {
      ...
      if (res?.status === 'failed') {
        ...
        failedVideoMessages.value = {
          ...failedVideoMessages.value,
          [storyboardId]: res?.error_msg || res?.errorMsg || t('dramaEpisode.toast.videoGenFailed'),
        }
...
function batchVideos() {
  ...
  if (!missing.length) {
    toast.info(t('dramaEpisode.toast.allShotsVideosGenerated'))
    return
  }
